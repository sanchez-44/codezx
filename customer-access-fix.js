(function(){
  ...
  function init(){var box=build();var btn=document.getElementById('customer-button');if(!box||!btn)return;btn.onclick=function(e){e.stopPropagation();box.classList.toggle('open');render();};document.addEventListener('click',function(e){if(!box.contains(e.target))box.classList.remove('open');});render();}
